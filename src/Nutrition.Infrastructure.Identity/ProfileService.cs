...
public sealed class ProfileService : IProfileService
{
    ...
    public ProfileService(IUserMealEntryRepository mealEntryRepository, IUserDailyGoalRepository goalRepository,
        UserManager<ApplicationUser> userManager, NutritionIdentityDbContext dbContext)
    {
    ...
    public async Task<ProfileResponseDto?> UpdateUserProfileAsync(Guid userId, UpdateProfileRequestDto request,
        CancellationToken cancellationToken = default)
    {
        var user = await _userManager.Users.FirstOrDefaultAsync(item => item.Id == userId, cancellationToken);
        if (user is null)
    ...
    public async Task<ProfileHistoryResponseDto> GetUserHistoryAsync(Guid userId,
        CancellationToken cancellationToken = default)
    {
    ...
    public async Task<NutritionSummaryDto> GetUserDailySummaryAsync(Guid userId,
        CancellationToken cancellationToken = default)
    {
        var entries =
            await _mealEntryRepository.GetByUserIdAndDateAsync(userId, DateTimeOffset.UtcNow, cancellationToken);
        return Sum(entries);
    }

    public async Task<ProfileSummaryByTypeResponseDto> GetUserSummaryByMealTypeAsync(Guid userId,
        CancellationToken cancellationToken = default)
    {
        var entries = await _mealEntryRepository.GetByUserIdAsync(userId, cancellationToken);
        var summaryByType = entries.GroupBy(entry => entry.MealType).OrderBy(group => group.Key).Select(group =>
        {
            var summary = Sum(group);
            return new MealEntrySummaryByTypeDto(group.Key.ToString(), summary.Calories, summary.Protein, summary.Fat,
                summary.Carbs, group.Count());
        }).ToArray();

        return new ProfileSummaryByTypeResponseDto(summaryByType, Sum(entries));
    }

    public async Task<ProfileDayResponseDto> GetUserDayAsync(Guid userId, DateOnly date, int utcOffsetMinutes = 0,
        CancellationToken cancellationToken = default)
    {
        ...
        var meals = Enum.GetValues<MealType>().Select(mealType =>
        {
            var mealEntries = entries.Where(entry => entry.MealType == mealType)
                .OrderByDescending(entry => entry.LoggedAtUtc).ToArray();

            return new MealEntriesByTypeDto(mealType.ToString(), mealEntries.Select(ToDto).ToArray(), Sum(mealEntries));
        }).ToArray();

        return new ProfileDayResponseDto(date, goal, meals, Sum(entries));
    }

    public async Task<ProfileStatisticsResponseDto> GetUserStatisticsAsync(Guid userId, int rangeDays, DateOnly endDate,
        CancellationToken cancellationToken = default)
    {
        ...
        var entries = await _mealEntryRepository.GetByUserIdAndRangeAsync(userId, utcStart, utcEnd, cancellationToken);
        var entriesByDate = entries.GroupBy(entry => DateOnly.FromDateTime(entry.LoggedAtUtc.UtcDateTime.Date))
            .ToDictionary(group => group.Key, group => group.ToArray());
        var goal = await GetUserDailyGoalAsync(userId, cancellationToken);

        var items = Enumerable.Range(0, normalizedRange).Select(offset =>
        {
            ...
            return new ProfileStatisticsDayDto(date, summary.Calories, summary.Protein, summary.Fat, summary.Carbs,
                SumMealCalories(dayEntries, MealType.Breakfast), SumMealCalories(dayEntries, MealType.Lunch),
                SumMealCalories(dayEntries, MealType.Dinner), SumMealCalories(dayEntries, MealType.Snack), true);
        }).ToArray();
    ...
    public async Task<string> ExportUserDailyCsvAsync(Guid userId, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var group in entries.GroupBy(entry => DateOnly.FromDateTime(entry.LoggedAtUtc.UtcDateTime.Date))
                     .OrderBy(group => group.Key))
        {
            var dayEntries = group.ToArray();
            var summary = Sum(dayEntries);
            builder.Append(group.Key.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture)).Append(',')
                .Append(ToCsvNumber(summary.Calories)).Append(',').Append(ToCsvNumber(summary.Protein)).Append(',')
                .Append(ToCsvNumber(summary.Fat)).Append(',').Append(ToCsvNumber(summary.Carbs)).Append(',')
    ...
    public async Task<UserDailyGoalDto?> GetUserDailyGoalAsync(Guid userId,
        CancellationToken cancellationToken = default)
    {
    ...
    public async Task<UserDailyGoalDto> CreateUserDailyGoalAsync(Guid userId, CreateDailyGoalRequestDto request,
        CancellationToken cancellationToken = default)
    {
        ...
        if (existing is not null)
        {
            existing.Update(request.TargetCalories, request.TargetProtein, request.TargetFat, request.TargetCarbs,
                request.BreakfastPercent ?? existing.BreakfastPercent, request.LunchPercent ?? existing.LunchPercent,
                request.DinnerPercent ?? existing.DinnerPercent, request.SnackPercent ?? existing.SnackPercent);
            await _goalRepository.UpdateAsync(existing, cancellationToken);
            await _goalRepository.SaveChangesAsync(cancellationToken);
            return ToDto(existing);
        }

        var goal = new UserDailyGoal(userId, request.TargetCalories, request.TargetProtein, request.TargetFat,
            request.TargetCarbs, request.BreakfastPercent ?? UserDailyGoal.DefaultBreakfastPercent,
    ...
    public async Task<UserDailyGoalDto> UpdateUserDailyGoalAsync(Guid userId, UpdateDailyGoalRequestDto request,
        CancellationToken cancellationToken = default)
    {
        var goal = await _goalRepository.GetByUserIdAsync(userId, cancellationToken);
        if (goal is null)
        {
            goal = new UserDailyGoal(userId, request.TargetCalories, request.TargetProtein, request.TargetFat,
                request.TargetCarbs, request.BreakfastPercent ?? UserDailyGoal.DefaultBreakfastPercent,
                request.LunchPercent ?? UserDailyGoal.DefaultLunchPercent,
                request.DinnerPercent ?? UserDailyGoal.DefaultDinnerPercent,
                request.SnackPercent ?? UserDailyGoal.DefaultSnackPercent);
        ...
        else
        {
            goal.Update(request.TargetCalories, request.TargetProtein, request.TargetFat, request.TargetCarbs,
                request.BreakfastPercent ?? goal.BreakfastPercent, request.LunchPercent ?? goal.LunchPercent,
                request.DinnerPercent ?? goal.DinnerPercent, request.SnackPercent ?? goal.SnackPercent);
    ...
    public async Task<UserMealEntryDto> CreateUserMealEntryAsync(Guid userId, CreateUserMealEntryRequestDto request,
        CancellationToken cancellationToken = default)
    {
        var mealType = ParseMealType(request.MealType);
        var entry = new UserMealEntry(Guid.NewGuid(), userId, request.ProductName, request.Brand ?? string.Empty,
            request.Calories, request.Protein, request.Fat, request.Carbs, mealType, Math.Max(0, request.ServingGrams),
            request.PortionLabel, request.SourceType, request.SourceReference,
    ...
    public async Task<UserMealEntryDto?> UpdateUserMealEntryAsync(Guid userId, Guid entryId,
        UpdateUserMealEntryRequestDto request, CancellationToken cancellationToken = default)
    {
        ...
        entry.Update(request.ProductName, request.Brand ?? string.Empty, request.Calories, request.Protein, request.Fat,
            request.Carbs, ParseMealType(request.MealType), Math.Max(0, request.ServingGrams), request.PortionLabel,
            request.SourceType, request.SourceReference, request.LoggedAtUtc ?? entry.LoggedAtUtc);
    ...
    public async Task DeleteUserAccountAsync(Guid userId, CancellationToken cancellationToken = default)
    {
        ...
        var goal = await _dbContext.UserDailyGoals.FirstOrDefaultAsync(item => item.UserId == userId,
            cancellationToken);
        if (goal is not null)
        {
    ...
    {
        return new UserMealEntryDto(entry.Id, entry.ProductName, entry.Brand, entry.Calories, entry.Protein, entry.Fat,
            entry.Carbs, entry.MealType.ToString(), entry.ServingGrams, entry.PortionLabel, entry.SourceType,
            entry.SourceReference, entry.LoggedAtUtc, entry.CreatedAtUtc);
    }
    ...
    private static UserDailyGoalDto ToDto(UserDailyGoal goal)
    {
        return new UserDailyGoalDto(goal.TargetCalories, goal.TargetProtein, goal.TargetFat, goal.TargetCarbs,
            goal.BreakfastPercent, goal.LunchPercent, goal.DinnerPercent, goal.SnackPercent);
    }

    private static NutritionSummaryDto Sum(IEnumerable<UserMealEntry> entries)
    {
        return new NutritionSummaryDto(entries.Sum(entry => entry.Calories), entries.Sum(entry => entry.Protein),
            entries.Sum(entry => entry.Fat), entries.Sum(entry => entry.Carbs));
    }
    ...
    {
        return Enum.TryParse<MealType>(value, ignoreCase: true, out var mealType) ? mealType : MealType.Snack;
    }
}
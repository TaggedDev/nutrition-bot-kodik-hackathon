...
public sealed class UserMealEntry
{
    ...
    public UserMealEntry(Guid id, Guid userId, string productName, string brand, decimal calories, decimal protein,
        decimal fat, decimal carbs, MealType mealType, DateTimeOffset createdAtUtc) : this(id, userId, productName,
        brand, calories, protein, fat, carbs, mealType, 0, DefaultPortionLabel, string.Empty, string.Empty,
    ...
    {
        if (id == Guid.Empty) throw new DomainValidationException("Entry id is required.");
        if (userId == Guid.Empty) throw new DomainValidationException("User id is required.");
        if (string.IsNullOrWhiteSpace(productName)) throw new DomainValidationException("Product name is required.");
        if (calories < 0 || protein < 0 || fat < 0 || carbs < 0)
            throw new DomainValidationException("Nutrition values cannot be negative.");
        if (servingGrams < 0) throw new DomainValidationException("Serving grams cannot be negative.");
    ...
    public void Update(string productName, string brand, decimal calories, decimal protein, decimal fat, decimal carbs,
        MealType mealType)
    {
        Update(productName, brand, calories, protein, fat, carbs, mealType, ServingGrams, PortionLabel, SourceType,
            SourceReference, LoggedAtUtc);
    }

...
public interface IUserMealEntryRepository
{
    ...
    Task<IReadOnlyCollection<UserMealEntry>> GetByUserIdAsync(Guid userId, CancellationToken cancellationToken = default);

    Task<IReadOnlyCollection<UserMealEntry>> GetByUserIdAndMealTypeAsync(Guid userId, MealType mealType, CancellationToken cancellationToken = default);
